...
- [x] **Codex Desktop exact-task delivery (opt-in)** — MCP calls made inside a Desktop task default to `codex:task:<thread-id>`; the macOS receive hook sends active-task guidance through the Desktop-native follow-up path and falls back to the shared `codex queue` command. Only the exact non-archived task is eligible. Legacy/unaddressed messages remain inbox-only, and synchronous `murmur_request` replies are not delivered twice. See [`docs/codex-desktop-queue-wake.md`](docs/codex-desktop-queue-wake.md).
