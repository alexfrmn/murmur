...
### Pending
- **NATS transport security (TLS + per-peer auth)** — #153 subsumes #103 and adds restricted pre-provisioned JetStream roles, per-user reply inboxes and a separate TLS dashboard role. It intentionally makes existing non-loopback `nats://` configurations fail closed. The canonical setup/package flow and current platform checks must be integrated before merge, and deployment remains held for the coordinated broker/client credential window in `docs/nats-transport-security.md`.
