...
Setup refuses an occupied shortcut belonging to another installation. It removes an old owned flat Programs/Murmur.lnk to avoid duplicate Start entries. The same ownership check is used for upgrade and cleanup. It recognizes either Windows PowerShell with -File and the quoted installed Open-Murmur.ps1 argument or the installed tray with empty arguments; both require the exact managed description and installation working directory. Foreign shortcuts are preserved.
