...
can contain paths. The detailed message stays in the service log
(`%ProgramData%\Murmur\logs\<service name>\`). Keys are case-sensitive and do not
depend on the helper language.
...
| `error.start` | The Service Control Manager refused to start the service. | Check the service log, then retry `service start` elevated. |
...
| `service.leftRunning` | Stop was requested but the service is still running. | Retry `service stop` elevated. |
...
| `error.unchanged` | The operation failed before changing anything. | Read the service log for the underlying cause and retry. |
...
| `error.mode` | The helper could not tell whether it runs as a service. | Retry; if it persists, report it with the service log. |
...
| `error.unknown` | The helper failed for a reason it has no key for. | Read the service log and report it with the code. |
