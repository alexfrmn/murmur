...
for (const dangling of [false, true]) test(`client preview/apply reject ${dangling ? 'dangling' : 'existing'} symlink without replacing it`, { skip: skipWithoutSymlinks }, async t => {
  const f = await fixture(t, 'json'); await profile(f);
