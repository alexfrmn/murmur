...
test('install backs up owned changes, refuses loaded updates and is idempotent', { skip: process.platform === 'win32' && 'requires POSIX paths, ownership or executable permissions' }, async t => {
...
test('foreign files and symlinks are never replaced', { skip: process.platform === 'win32' && 'requires POSIX paths, ownership or executable permissions' }, async t => {
...
test('same label cannot start or stop a different contour', { skip: process.platform === 'win32' && 'requires POSIX paths, ownership or executable permissions' }, async t => {
  const f = await fixture(t); await f.adapter.install(f.context); f.loaded();
...
test('service operations reject controls and conflicting derived paths before commands', { skip: process.platform === 'win32' && 'requires POSIX paths, ownership or executable permissions' }, async t => {
...
test('unit escaping keeps environment dollars literal and ExecStart dollars escaped', { skip: process.platform === 'win32' && 'requires POSIX paths, ownership or executable permissions' }, async t => {
  const f = await fixture(t);
